//url:logautonewtarget //url:logauto.js
...
function checkHot(table, side = false) {
    var data = tableStreak[table];
...
function readStreak(table, numbers, side = false) {
        if (!isNumeric(table.toString())) {
            return false;
        }
        tableStreak[table] = [];
        for (var i = numbers.length - 5; i > 0; i--) {
            var dist = findDistance(numbers[i], numbers[i - 1]);
            var r = countPockets(numbers[i - 1], dist[0], dist[1]);
            if (side) {
                r = countPockets(r, 18);
            }
            if (i - 2 >= 0) {
                tableStreak[table].push(doPlace(r, 7).indexOf(numbers[i - 2]) != -1);
            }
        }
    }

function readHotAndStreak(tid) {
        if (!isNumeric(tid.toString())) {
            return false;
        }
        readStreak(tid, tableNumbers[tid]);
        var ret = checkHot(tid);
        tablesHotStreak[tid] = [ret, false];
        document.getElementById('hotstreak' + tid).style.visibility = 'hidden';
        if (!ret) {
            readStreak(tid, tableNumbers[tid], true);
            ret = checkHot(tid, true);
            tablesHotStreak[tid] = [ret, true];
            if (ret) {
                document.getElementById('hotstreak' + tid).style.visibility = '';
            }
        } else {
            document.getElementById('hotstreak' + tid).style.visibility = '';
        }
    ...
    socket.on('newConnections', function(data) {
        document.getElementById('onlineusers').innerHTML = data;
    }); // Listen for messages 
    socket.on('pragmaticTablesUpdate', function(data) {
        //console.log(data);
        ...
        if (tid != -1) {
            ...
            if (cspins == 0) {
                document.getElementById(`cts${tid}`).innerHTML = '';
            } else {
                document.getElementById(`cts${tid}`).innerHTML = ``;
            }
        }
...
function jointable(elm, tid) {
        ...
        if (tableJoined != null) {
            SwitchSide = false;
            if (tablesHotStreak[tid] && tablesHotStreak[tid][0]) {
                SwitchSide = tablesHotStreak[tid][1];
            }
    ...
    function calculatePrediction(n1, n2, diff = 0) {
        if (lastPrediction != -1) {
            var won = doPlace(lastPrediction, 7).indexOf(n2) != -1;
            totalP++;
            if (won) {
                winsP++;
                WLStreak.push('W');
...
function predictNext(outcomes, maxPatternLength = 5) {
        //Convert array to string  for easier pattern matching
...
function lookForPrediction() {
    ...
    if (WLStreak.length == 9) {
        ...
        if (Object.keys(resultPattern.details).indexOf('W') != -1) {
            morextra += `
        W(${resultPattern.details.W})`;
        }

        if (Object.keys(resultPattern.details).indexOf('L') != -1) {
            morextra += `,
        L(${resultPattern.details.L})`;
        }
